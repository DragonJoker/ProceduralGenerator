#include "ProceduralGenerator/PrecompiledHeader.h"

#include "ProceduralGenerator/GeneratorVoronoi.h"

#define M_SQRT2 1.4142135623
#define M_SQRT3 1.7320508075
...
Voronoi::Thread :: Thread( ProceduralGenerator * p_pParent, size_t p_uiIndex, int iWidth, int iTop, int iBottom, int iTotalHeight, const UbPixel & p_pxColour)
...
Voronoi::Thread :: ~Thread()
{
}

void Voronoi::Thread :: Step()
{
	if (m_bEnded && m_bLaunched && ! m_bStopped)
	{
		m_bEnded = false;
		...
		for (int y = m_iTop ; y < m_iBottom ; y++)
		{
			...
			{
				Get( x, y, m_iStep, m_eType, ( * m_pixels)[x][y]);
			}
		}
...
double Voronoi::Thread :: Get( double x, double y, double z, eVORONOI_TYPE p_eType, UbPixel & p_rPixel)
{
	double d;
	double da[4];
	double pa[12];
	...
	case eCrackle:
		d = std::max( 1.0, 10.0 * Get( x, y, z, eDifference21, p_rPixel));
		break;
	...
	int iColour = (int( d * 255.0f) << 1) | 0x80;

	p_rPixel.a = 255;
	p_rPixel.r = static_cast<unsigned char>( iColour);
	p_rPixel.g = static_cast<unsigned char>( iColour);
	p_rPixel.b = static_cast<unsigned char>( iColour);
...
const float* Voronoi::Thread :: _getPoint( int x, int y, int z)
{
	const unsigned char Index[512]=
	{
...
Voronoi :: Voronoi( int p_width, int p_height, eVORONOI_TYPE p_eType, eDISTANCE_METHOD p_eDistance)
	:	ProceduralGenerator( p_width, p_height),
		m_eType( p_eType),
		m_eDistance( p_eDistance)
{
	int l_iOffset = m_iHeight / m_uiThreadCount;

	Voronoi::Thread * l_pThread;

	for (int i = 0 ; i < m_iHeight ; i += l_iOffset)
	{
		l_pThread = CreateThread<Voronoi::Thread>( m_iWidth, i, i + l_iOffset, m_iHeight, m_pxColour);
		l_pThread->m_eType = m_eType;
		l_pThread->m_eDistance = m_eDistance;
		l_pThread->Run();
	}
...
void Voronoi :: InitialiseStep()
{
	for (size_t i = 0 ; i < m_arraySlaveThreads.size() ; i++)
	{
		if (m_arraySlaveThreads[i] != NULL)
		{
			((Voronoi::Thread *)m_arraySlaveThreads[i])->SetBuffer( & m_backBuffer);
			m_arraySlaveThreads[i]->Relaunch();
		}
	}
}

void Voronoi :: SetType( eVORONOI_TYPE p_eType)
{
	m_eType = p_eType;
	...
	for (size_t i = 0 ; i < m_arraySlaveThreads.size() ; i++)
	{
		if (m_arraySlaveThreads[i] != NULL)
		{
			((Voronoi::Thread *)m_arraySlaveThreads[i])->m_eType = p_eType;
		}
	}
}

void Voronoi :: SetDistance( eDISTANCE_METHOD p_eDistance)
{
	m_eDistance = p_eDistance;
	...
	for (size_t i = 0 ; i < m_arraySlaveThreads.size() ; i++)
	{
		if (m_arraySlaveThreads[i] != NULL)
		{
			((Voronoi::Thread *)m_arraySlaveThreads[i])->m_eDistance = p_eDistance;
		}
	}
}
